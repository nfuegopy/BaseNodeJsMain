...
router.get('/user-details', async (req, res) => {
  ...
  const [rows] = await pool.execute('CALL ObtenerNombreUsuario(?, @nombre, @apellido, @usuarioID)', [username]);
  const [resultado] = await pool.execute('SELECT @nombre, @apellido, @usuarioID');
  res.json({
